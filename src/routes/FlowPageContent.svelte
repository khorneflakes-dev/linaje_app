<script lang="ts">
  ...
  import { getLayoutedElements, getHiddenDescendants, getAncestors } from '$lib/utils/graphUtils';
  ...
  function onNodeClick({ node }: { node: Node }) {
    ...
    const highlightSet = new Set([nodeId, ...ancestors]);

    nodes.update(ns => ns.map(n => ({
      ...n,
      data: { ...n.data, highlighted: highlightSet.has(n.id) }
    })));

    edges.update(es => es.map(e => {
      const isHighlighted = highlightSet.has(e.target) && highlightSet.has(e.source);
      return {
        ...e,
        animated: isHighlighted,
        style: isHighlighted ? 'stroke: #3b82f6; stroke-width: 2px;' : ''
      };
    }));
  }
